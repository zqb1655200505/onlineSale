...
public class FileUtils extends org.apache.commons.io.FileUtils {
	...
	public static String downFile(File file, HttpServletRequest request, HttpServletResponse response, String fileName){
		...
		try {
			response.addHeader("Content-Disposition", "attachment; filename=\"" +
					URLEncoder.encode(StringUtils.isBlank(fileName) ? file.getName() : fileName) + "\"");
	...
	public static String writeToServer(HttpServletRequest request,String directory, MultipartFile file)
	{
		String basePath=request.getSession().getServletContext().getRealPath("/");
		File dir=new File(basePath+directory);
		if(!dir.exists())
		{
			dir.mkdirs();
		}
